...
<nav class="bg-white border-gray-200 dark:bg-gray-900">
  ...
  <div class="flex items-center md:order-2">
      <button v-if="isLoggedIn" type="button" class="flex mr-3 text-sm bg-gray-800 rounded-full md:mr-0 focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600" id="user-menu-button" aria-expanded="false" data-dropdown-toggle="user-dropdown" data-dropdown-placement="bottom">
        <span class="sr-only">Open user menu</span>
        <img class="w-8 h-8 rounded-full" :src="userData.photoURL" alt="user photo">
      </button>
      <!-- Dropdown menu -->
      <div class="z-50 hidden my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow dark:bg-gray-700 dark:divide-gray-600" id="user-dropdown">
        <div class="px-4 py-3">
          <span class="block text-sm text-gray-900 dark:text-white">{{ userData.displayName }}</span>
        ...
        <ul class="py-2" aria-labelledby="user-menu-button">
          ...
          <li>
            <button @click="handleSignOut" class="block py-2 pl-3 pr-4 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent">Logout</button>
          </li>
        </ul>
      </div>
  ...
  <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="mobile-menu-2">
    ...
    <main class="py-4 flex justify-center w-full">
    <router-view />
    </main>
  </div>
</template>
...
import { auth, onAuthStateChanged, signOut } from './firebaseDb'
import { useRouter } from 'vue-router'
import { initDropdowns } from 'flowbite'

document.addEventListener('DOMContentLoaded', function () {
  // Inicializa todos los dropdowns en el documento
  initDropdowns()
})
